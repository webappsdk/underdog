var UNDERDOG_PATH="";function setUnderdogPath(a){UNDERDOG_PATH=a}function loadIntoEl(a,b,c){lockScreen();for(var d=document.getElementsByClassName("ckeditor"),e=0;e<d.length;e++){var f=d.eq(e).attr("id");(f=CKEDITOR.instances[f])&&CKEDITOR.remove(f)}loadDataIntoEl(a,b,c)}function loadDataIntoEl(a,b,c){CRUD.load(a,function(a){b.html(a);b.trigger("create");unlockScreen();hideLoader();"undefined"!=typeof c&&c()})}
...
Alert.yesno=function(a,b){var c=confirm(a);b(c)};function PH(){}PH.EVENT_PLUGIN_JSON_FILE="plugins.json";PH.CACHED_CONFIGURATIONS_ID="PH.configurations";PH.CACHED_DESCRIPTOR_ID="PH.descriptor";PH.LOAD_PH_AFTER_EVENT="load-ph-after";PH.root=null;PH.events={};
PH.load=function(a,b){CH(b);CH.parseObjects();PH.root=a;U.un(CH.cache(PH.CACHED_CONFIGURATIONS_ID))&&CH.cache(PH.CACHED_CONFIGURATIONS_ID,{});var c=function(a){new Promise(function(b,c){PH.loadPlugins(PH.LOAD_PH_AFTER_EVENT,"eager");PH.loadPlugins(PH.LOAD_PH_AFTER_EVENT,"lazy");for(var d in a)d!=PH.LOAD_PH_AFTER_EVENT&&PH.loadPlugins(d,"eager");b()})},d=CH.cache(PH.CACHED_DESCRIPTOR_ID);U.un(d)?(a+=PH.EVENT_PLUGIN_JSON_FILE,CRUD.loadJSON(a,function(a){CH.cache(PH.CACHED_DESCRIPTOR_ID,a);c(a)})):c(d)};
...
PH.wrappedRun=function(a,b,c,d){PH.fire(a.id+"-before",b,function(b){(new Promise(function(d,h){PH.fire(a.id+"-in-process",b);var g=CH.cache(PH.CACHED_CONFIGURATIONS_ID)[a.id];U.un(g)&&(g={});try{var k=a.run(b,g,c);d(k)}catch(l){h("error_plugin_execution.")}})).then(function(a){U.un(d)||d(a)},function(a){U.un(d)||d({success:!1,message:a})})["finally"](function(){PH.fire(a.id+"-after",b)})})};function EM(){}EM.editors={};EM.activeEditor=null;EM.popup=null;EM.popupBody=null;EM.field=null;
EM.load=function(a,b){EM.field=b;U.un(EM.editors[a])?U.loadjs("/editor/"+a+".js"):EM.render(EM.editors[a])};EM.add=function(a,b){null!=b&&(EM.field=b);EM.editors[a.id]=a;EM.render(a)};
