...
U.loadjs=function(a,b){if(!U.un(a))if(U.un(b)){var c=document.createElement("script");c.type="application/javascript";c.src=a;document.body.appendChild(c)}else{var d=document.createElement("script"),e=!1,f=document.body;d.onload=d.onreadystatechange=function(){e||this.readyState&&"loaded"!==this.readyState&&"complete"!==this.readyState||(e=!0,b(),d.onload=d.onreadystatechange=null,f&&d.parentNode&&f.removeChild(d))};d.setAttribute("type","text/javascript");d.setAttribute("src",a);f.appendChild(d)}};
...
U.onMouseMove(function(a){U.mouseX=a.clientX;U.mouseY=a.clientY});Promise.prototype["finally"]=function(a){return"function"!==typeof a?this.then():this.then(a,a)};function Tooltip(){}Tooltip["new"]=function(a){if(null==document.getElementById(a)){var b=document.createElement("span");b.setAttribute("id","underdog-tooltip-"+a);b.setAttribute("style","border:2px solid #444;padding:5px;background-color:#fff;display:none;position:fixed;overflow:hidden;");document.body.appendChild(b)}};
...
Alert.yesno=function(a,b){var c=confirm(a);b(c)};function PH(){}PH.CACHED_CONFIGURATIONS_ID="PH.configurations";PH.CACHED_RESOURCES_ID="PH.resources";PH.EVENT_PLUGIN_JSON_FILE="plugins.json";PH.CACHED_DESCRIPTOR_ID="PH.descriptor";PH.INIT_PH_AFTER_EVENT="init-ph-after";PH.root=null;PH.events={};PH.plugins={};
PH.init=function(a){PH.root=a;U.un(CH.cache(PH.CACHED_CONFIGURATIONS_ID))&&CH.cache(PH.CACHED_CONFIGURATIONS_ID,{});U.un(CH.cache(PH.CACHED_RESOURCES_ID))&&CH.cache(PH.CACHED_RESOURCES_ID,{});var b=function(a){(new Promise(function(b,c){for(var g in a)g!=PH.INIT_PH_AFTER_EVENT&&PH.loadPlugins(g,"eager");b()}))["finally"](function(){PH.fire(PH.INIT_PH_AFTER_EVENT)})},c=CH.cache(PH.CACHED_DESCRIPTOR_ID);U.un(c)?(a+=PH.EVENT_PLUGIN_JSON_FILE,CRUD.loadJSON(a,function(a){CH.cache(PH.CACHED_DESCRIPTOR_ID,
a);b(a)})):b(c)};
PH.loadPlugins=function(a,b,c){if(null==b||"eager"!=b&&"lazy"!=b)b="lazy";var d=CH.cache(PH.CACHED_DESCRIPTOR_ID);if(d.hasOwnProperty(a)){if(a=d[a],0<a.length){var e=function(a,b,c,d){null==d&&(d=0);a.length>d?(plugin=a[d],(plugin.hasOwnProperty("load")&&plugin.load==b||!plugin.hasOwnProperty("load")&&"lazy"==b)&&plugin.hasOwnProperty("package")&&plugin.hasOwnProperty("plugin")&&!PH.plugins.hasOwnProperty(plugin["package"]+"."+plugin.plugin)?PH.loadPlugin(plugin,function(){d++;e(a,b,c,d)}):(d++,e(a,
b,c,d))):U.un(c)||c()};e(a,b,c)}}else U.un(c)||c()};PH.loadPlugin=function(a,b){PH.loadPluginConfiguration(a,function(){U.loadjs(PH.root+a["package"]+"/client/"+a.plugin+".js",b)})};PH.add=function(a,b){var c=a.events;if(U.un(c)||0==c.length)PH.wrappedRun(a,b,null);else{for(var d=null,e=0;e<c.length;e++)d=c[e],U.un(PH.events[d])&&(PH.events[d]=[]),PH.events[d].push(a.id);U.un(a.id)||(PH.plugins[a.id]=a)}};
PH.loadPluginConfiguration=function(a,b){var c=CH.cache(PH.CACHED_CONFIGURATIONS_ID),d=a["package"]+"."+a.plugin;c.hasOwnProperty(d)?b(a):CRUD.loadJSON(PH.root+"/"+a["package"]+"/configuration.json",function(a){c[d]=a;CH.cache(PH.CACHED_CONFIGURATIONS_ID,c);b(plugin)})};PH.fire=function(a,b,c){PH.loadPlugins(a,"lazy",function(){if(PH.events.hasOwnProperty(a))for(var d=PH.events[a],e=null,f=0;f<d.length;f++)e=PH.getPluginById(d[f]),null!=e&&PH.wrappedRun(e,b,a,c);else U.un(c)||c(b)})};
PH.wrappedRun=function(a,b,c,d){PH.fire(a.id+"-before",b,function(b){(new Promise(function(d,g){PH.fire(a.id+"-in-process",b);var h=CH.cache(PH.CACHED_CONFIGURATIONS_ID)[a.id];U.un(h)&&(h={});try{var k=a.run(b,h,c);d(k)}catch(l){g("error_plugin_execution.")}})).then(function(a){U.un(d)||d(a)},function(b){U.un(d)||d({success:!1,message:b,data:{pluginid:a.id}})})["finally"](function(){PH.fire(a.id+"-after",b)})})};PH.getPluginById=function(a){return PH.plugins.hasOwnProperty(a)?PH.plugins[a]:null};
PH.remove=function(a){var b=null;"object"==U.type(a)&&(b=a.id);if("string"==U.type(b)&&PH.plugins.hasOwnProperty(b)){a=PH.plugins[b].events;for(var c=null,d=0;d<a.length;d++)c=a[d],c=PH.events.indexOf(c),-1<c&&array.splice(c,1);delete PH.plugins[b]}};PH.sendMessage=function(a,b,c,d){if(U.un(c)){var e=null,f;for(f in PH.plugins)if(e=PH.plugins[f],"function"==typeof e.onMessage)e.onMessage(a,b,d)}else for(f=0;f<c.length;f++)if(e=PH.getPluginById[c[f]],null!=e)e.onMessage(a,b,d)};
PH.getResourcePath=function(a){try{var b=a.id.split(".");return PH.root+"/"+b[0]+"/resources/"}catch(c){return""}};function EM(){}EM.editors={};EM.activeEditor=null;EM.popup=null;EM.popupBody=null;EM.field=null;EM.load=function(a,b){EM.field=b;U.un(EM.editors[a])?U.loadjs("/editor/"+a+".js"):EM.render(EM.editors[a])};EM.add=function(a,b){null!=b&&(EM.field=b);EM.editors[a.id]=a;EM.render(a)};
...
CRUD.load=function(a,b,c,d,e){var f;if("undefined"!==typeof XMLHttpRequest)f=new XMLHttpRequest;else for(var g=["MSXML2.XmlHttp.5.0","MSXML2.XmlHttp.4.0","MSXML2.XmlHttp.3.0","MSXML2.XmlHttp.2.0","Microsoft.XmlHttp"],h=0,k=g.length;h<k;h++)try{f=new ActiveXObject(g[h]);break}catch(l){}f.onreadystatechange=function(){if(!(4>f.readyState)&&200===f.status&&4===f.readyState)if(U.un(c))b(f.responseText);else try{b(JSON.parse(f.responseText))}catch(a){b({})}};U.un(d)&&(d="GET");f.open(d,a,!0);if(U.un(e))f.send();
